use fastfile_benches::{
    benches::{cleanup, methods::fastfile::fastread::read, prepare, FILE_SIZES_VERY_SMALL},
    benchmark::Benchmark,
};
...
fn main() {
    ...
    let benchmark = Benchmark::new(benchmark_name, &params, iterations)
        ...
        .add_func("fastread", |p| {
            read(p, Some(*FILE_SIZES_VERY_SMALL.last().unwrap())) // Safe, because slice is not empty
        });
