...
fn bench_impls(c: &mut Criterion) {
    ...
    c.bench(
        ...
        ParameterizedBenchmark::new(
            "stdlib_buf_read",
            |b, param| b.iter(|| methods::stdlib::buf_read::read(&param.path, true)),
            params,
        )
        .with_function("read", |b, param| {
            b.iter(|| methods::fastfile::read::read(&param.path, true))
        })
        .with_function("fastread", |b, param| {
            b.iter(|| methods::fastfile::fastread::read(&param.path, true))
        })
